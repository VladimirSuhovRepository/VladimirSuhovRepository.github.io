...
(function (jQuery, undefined) {
    ...
    function initSlider(container, opt) {
        ...
        if (!container.hasClass("revslider-initialised")) {
            ...
            var version = jQuery.fn.jquery.split('.'), versionTop = parseFloat(version[0]), versionMinor = parseFloat(version[1]), versionIncrement = parseFloat(version[2] || '0');
    ...
    ********************************/
    ...
    var createArrows = function (container, opt) {
        ...
        var hidden = "", starthidearrows = "hidearrows";
        if (opt.hideThumbs == 0)
            starthidearrows = "";
    ...
    *******************************************************/
    var handleSpecialPreviews = function (opt) {
        ...
        if (opt.comingli >= opt.slideamount)
            opt.comingli = 0;
        var comingli = container.find('>ul:first-child >li:eq(' + opt.comingli + ')'), beforli = container.find('>ul:first-child >li:eq(' + opt.beforli + ')'), previmgsrc = beforli.find('.defaultimg').attr('src'), nextimgsrc = comingli.find('.defaultimg').attr('src');
        // SAVE REFERENCES
        ...
        // READ REFERENCES
        var arr = opt.arr, rar = opt.rar, lar = opt.lar, raimg = opt.raimg, laimg = opt.laimg, raimg_b = opt.raimg_b, laimg_b = opt.laimg_b, ratit = opt.ratit, latit = opt.latit;
        ...
        if (arr.hasClass("preview2") && !arr.hasClass("hashoveralready")) {
            ...
            if (!is_mobile())
                arr.hover(function () {
                    var arr = jQuery(this), th = arr.find('.tp-arr-titleholder');
    ...
    var prepareSlides = function (container, opt) {
        ...
        container.find('>ul:first >li >img').each(function (j) {
            ...
            var src = img.attr('src'), ll = img.data('lazyload'), bgfit = img.data('bgfit'), bgrepeat = img.data('bgrepeat'), bgposition = img.data('bgposition');
            if (bgfit == undefined)
                bgfit = "cover";
    ...
    var prepareOneSlideSlot = function (slotholder, opt, visible, vorh) {
        var sh = slotholder, img = sh.find('.defaultimg'), scalestart = sh.data('zoomstart'), rotatestart = sh.data('rotationstart');
        if (img.data('currotate') != undefined)
            rotatestart = img.data('currotate');
        if (img.data('curscale') != undefined)
            scalestart = img.data('curscale');
        setSize(img, opt);
        var src = img.data('src'), bgcolor = img.css('background-color'), w = opt.width, h = opt.height, fulloff = img.data("fxof");
        if (opt.autoHeight == "on")
            h = opt.container.height();
        if (fulloff == undefined)
            fulloff = 0;
        fullyoff = 0;
        var off = 0, bgfit = img.data('bgfit'), bgrepeat = img.data('bgrepeat'), bgposition = img.data('bgposition');
        if (bgfit == undefined)
            bgfit = "cover";
        if (bgrepeat == undefined)
            bgrepeat = "no-repeat";
    ...
    ***********************************************/
    var ieimgposition = function (nextsh, opt) {
        if (isIE(8)) {
            ...
            var ie8w = ie8img.width(), ie8h = ie8img.height();
    ...
    *******************************************/
    var slideAnimation = function (container, opt, nextli, actli, actsh, nextsh) {
        ...
        ///////////////////////////////////////
        var nexttrans = 0, transtext = nextli.data('transition').split(","), curtransid = nextli.data('nexttransid');
        ...
        */
        ...
        if (nexttrans == 4 || nexttrans == 5) {
            ...
            // ALL NEW SLOTS SHOULD BE SLIDED FROM THE LEFT TO THE RIGHT
            var cspeed = (masterspeed) / 1000, ticker = cspeed, subtl = new punchgs.TimelineLite();
        ...
        if (nexttrans == 16) {
            ...
            var ro1 = Math.round(Math.random() * 20 - 10), ro2 = Math.round(Math.random() * 20 - 10), ro3 = Math.round(Math.random() * 20 - 10), xof = Math.random() * 0.4 - 0.2, yof = Math.random() * 0.4 - 0.2, sc1 = Math.random() * 1 + 1, sc2 = Math.random() * 1 + 1, sc3 = Math.random() * 0.3 + 0.3;
            ...
            if (actli.html() != null)
                mtl.add(punchgs.TweenLite.fromTo(nextli, (masterspeed - 200) / 1000, { scale: sc3, x: (opt.width / 4) * xof, y: (ooh / 4) * yof, rotation: ro3, force3D: "auto", transformOrigin: "center center", ease: punchgs.Power2.easeOut }, { autoAlpha: 1, scale: 1, x: 0, y: 0, rotation: 0 }), 0);
            mtl.add(subtl, 0);
        }
        ////////////////////////////////////////
        ...
        if (nexttrans == 19 || nexttrans == 22) {
            ...
            var chix = nextli.css('z-index'), chix2 = actli.css('z-index'), rot = 90, op = 1;
            if (direction == 1)
                rot = -90;
            if (nexttrans == 19) {
    ...
    **************************************/
    ...
    var animateTheCaptions = function (nextli, opt, recalled, mtl) {
        ...
        var ctl = nextli.data('ctl'), offsetx = 0, offsety = 0, allcaptions = nextli.find('.tp-caption'), allstaticcaptions = opt.container.find('.tp-static-layers').find('.tp-caption');
        ctl.pause();
        jQuery.each(allstaticcaptions, function (index, staticcapt) {
            allcaptions.push(staticcapt);
        });
        allcaptions.each(function (i) {
            var internrecalled = recalled, staticdirection = -1, // 1 -> In,  2-> Out  0-> Ignore  -1-> Not Static
            ...
            if (handlecaption == 0) {
                        ...
                        if (ifr.attr('src').toLowerCase().indexOf('youtube') >= 0) {
                            ...
                            if (ifr.attr('src').toLowerCase().indexOf('vimeo') >= 0) {
                                if (!ifr.hasClass("HasListener")) {
                                    ...
                                    var queryParameters = {}, queryString = isrc, re = /([^&=]+)=([^&]*)/g, m;
                ...
                if (!internrecalled) {
                    ...
                    else if (nextcaption.hasClass('lfr') || nextcaption.hasClass('skewfromright'))
                        frm.x = 15 + opt.width;
                    else if (nextcaption.hasClass('lfl') || nextcaption.hasClass('skewfromleft'))
                        frm.x = -15 - imw;
                    else if (nextcaption.hasClass('sfl') || nextcaption.hasClass('skewfromleftshort'))
                        frm.x = -50;
                    else if (nextcaption.hasClass('sfr') || nextcaption.hasClass('skewfromrightshort'))
                        frm.x = 50;
                    else if (nextcaption.hasClass('lft'))
                        frm.y = -25 - imh;
                    else if (nextcaption.hasClass('lfb'))
                        frm.y = 25 + opt.height;
                    else if (nextcaption.hasClass('sft'))
                        frm.y = -50;
                    else if (nextcaption.hasClass('sfb'))
                        frm.y = 50;
                    if (nextcaption.hasClass('skewfromright') || nextcaption.hasClass('skewfromrightshort'))
                        frm.skewX = -85;
                    else if (nextcaption.hasClass('skewfromleft') || nextcaption.hasClass('skewfromleftshort'))
                        frm.skewX = 85;
    ...
    ********************************/
    var callCaptionLoops = function (nextcaption, factor) {
        ...
        nextcaption.find('.rs-pendulum').each(function () {
            ...
            if (el.data('timeline') == undefined) {
                ...
                var startdeg = el.data('startdeg') == undefined ? -20 : el.data('startdeg'), enddeg = el.data('enddeg') == undefined ? 20 : el.data('enddeg');
        ...
        nextcaption.find('.rs-slideloop').each(function () {
            ...
            if (el.data('timeline') == undefined) {
                el.data('timeline', new punchgs.TimelineLite);
                var xs = el.data('xs') == undefined ? 0 : el.data('xs'), ys = el.data('ys') == undefined ? 0 : el.data('ys');
        ...
        nextcaption.find('.rs-pulse').each(function () {
            ...
            if (el.data('timeline') == undefined) {
                el.data('timeline', new punchgs.TimelineLite);
                var zoomstart = el.data('zoomstart') == undefined ? 0 : el.data('zoomstart'), zoomend = el.data('zoomend') == undefined ? 0 : el.data('zoomend');
                speed = el.data('speed') == undefined ? 2 : el.data('speed'),
                    easing = el.data('ease') == undefined ? punchgs.Power2.easeInOut : el.data('ease');
        ...
        nextcaption.find('.rs-wave').each(function () {
            ...
            if (el.data('timeline') == undefined) {
                el.data('timeline', new punchgs.TimelineLite);
                var angle = el.data('angle') == undefined ? 10 : el.data('angle'), radius = el.data('radius') == undefined ? 10 : el.data('radius'), speed = el.data('speed') == undefined ? -20 : el.data('speed'), origin = el.data('origin') == undefined ? -20 : el.data('origin');
                angle = angle * factor;
    ...
    var removeTheCaptions = function (actli, opt) {
        var removetime = 0;
        var allcaptions = actli.find('.tp-caption'), allstaticcaptions = opt.container.find('.tp-static-layers').find('.tp-caption');
        jQuery.each(allstaticcaptions, function (index, staticcapt) {
            allcaptions.push(staticcapt);
        });
        allcaptions.each(function (i) {
    ...
    var endMoveCaption = function (nextcaption, opt, mdelay, backwards, frame, downscale) {
        ...
        if (nextcaption.hasClass('ltr') ||
            ...
            if (nextcaption.hasClass('skewtoright') || nextcaption.hasClass('skewtorightshort'))
                frm.skewX = 35;
            else if (nextcaption.hasClass('skewtoleft') || nextcaption.hasClass('skewtoleftshort'))
                frm.skewX = -35;
            ...
            if (nextcaption.hasClass('skewtorightshort'))
                frm.x = 270;
            else if (nextcaption.hasClass('skewtoleftshort'))
                frm.x = -270;
            frm.data = new Object();
            frm.data.oldx = frm.x;
            frm.data.oldy = frm.y;
    ...
    ********************************/
    var startKenBurn = function (container, opt, recalc, prepareonly) {
        ...
        opt.lastslide = opt.act;
        var nextli = container.find('>ul:first-child >li:eq(' + opt.next + ')'), nextsh = nextli.find('.slotholder'), bgps = nextsh.data('bgposition'), bgpe = nextsh.data('bgpositionend'), zos = nextsh.data('zoomstart') / 100, zoe = nextsh.data('zoomend') / 100, ros = nextsh.data('rotationstart'), roe = nextsh.data('rotationend'), bgfs = nextsh.data('bgfit'), bgfe = nextsh.data('bgfitend'), easeme = nextsh.data('easeme'), dur = nextsh.data('duration') / 1000, bgfb = 100;
        if (bgfs == undefined)
            bgfs = 100;
        if (bgfe == undefined)
            bgfe = 100;
        var obgfs = bgfs, obgfe = bgfe;
        ...
        nextsh.find('.defaultimg').each(function () {
            ...
            var imgs = calculateKenBurnImgPos(opt, bgps, bgfs, kbimg, turned), imge = calculateKenBurnImgPos(opt, bgpe, bgfe, kbimg, turned);
            if (turned) {
                imgs.w = obgfs / 100;
                imge.w = obgfe / 100;
            }
            if (prepareonly) {
                punchgs.TweenLite.set(kbimg, { autoAlpha: 0,
                    ...
                    x: imgs.x,
                    y: imgs.y });
                var sx = imgs.w, ww = (sx * kbimg.width()) - opt.width, hh = (sx * kbimg.height()) - opt.height, hor = Math.abs((imgs.x / ww) * 100), ver = Math.abs((imgs.y / hh) * 100);
                if (hh == 0)
                    ver = 0;
                if (ww == 0)
                    hor = 0;
                ...
                defimg.data('kenburn', punchgs.TweenLite.fromTo(kbimg, dur, { autoAlpha: 1, force3D: punchgs.force3d, transformOrigin: "0% 0%", top: 0, left: 0, scale: imgs.w, x: imgs.x, y: imgs.y }, { autoAlpha: 1, rotationZ: roe, ease: easeme, x: imge.x, y: imge.y, scale: imge.w, onUpdate: function () {
                        ...
                        var ww = (sx * kbimg.width()) - opt.width, hh = (sx * kbimg.height()) - opt.height, hor = Math.abs((kbimg[0]._gsTransform.x / ww) * 100), ver = Math.abs((kbimg[0]._gsTransform.y / hh) * 100);
                        if (hh == 0)
                            ver = 0;
                        if (ww == 0)
                            hor = 0;
    ...
    var checkForParallax = function (container, opt) {
        ...
        if (opt.parallax == "mouse" || opt.parallax == "scroll+mouse" || opt.parallax == "mouse+scroll") {
            container.mouseenter(function (event) {
                ...
                var t = container.offset().top, l = container.offset().left, ex = (event.pageX - l), ey = (event.pageY - t);
            ...
            container.on('mousemove.hoverdir, mouseleave.hoverdir', function (event) {
                ...
                switch (event.type) {
                    case "mousemove":
                        var t = container.offset().top, l = container.offset().left, mh = currslide.data("enterx"), mv = currslide.data("entery"), diffh = (mh - (event.pageX - l)), diffv = (mv - (event.pageY - t));
                        currslide.find(".tp-parallax-container").each(function () {
                            var pc = jQuery(this), pl = parseInt(pc.data('parallaxlevel'), 0) / 100, offsh = diffh * pl, offsv = diffv * pl;
            ...
            if (is_mobile())
                window.ondeviceorientation = function (event) {
                    var y = Math.round(event.beta || 0), x = Math.round(event.gamma || 0);
                    var currslide = container.find('.current-sr-slide-visible');
                    ...
                    var curh = 360 / container.width() * x, curv = 180 / container.height() * y;
                    currslide.find(".tp-parallax-container").each(function () {
                        var pc = jQuery(this), pl = parseInt(pc.data('parallaxlevel'), 0) / 100, offsh = curh * pl, offsv = curv * pl;
    ...
    ***************************************/
    var scrollParallax = function (container, opt) {
        if (is_mobile() && opt.parallaxDisableOnMobile == "on")
            return false;
        var t = container.offset().top, st = jQuery(window).scrollTop(), dist = t + container.height() / 2, mv = t + container.height() / 2 - st, wh = jQuery(window).height() / 2, diffv = wh - mv;
        if (dist < wh)
            diffv = diffv - (wh - dist);
        var currslide = container.find('.current-sr-slide-visible');
        container.find(".tp-parallax-container").each(function (i) {
            var pc = jQuery(this), pl = parseInt(pc.data('parallaxlevel'), 0) / 100, offsv = diffv * pl;
            pc.data('parallaxoffset', offsv);
            punchgs.TweenLite.to(pc, 0.2, { force3D: "auto", y: offsv, ease: punchgs.Power3.easeOut });
        });
        if (opt.parallaxBgFreeze != "on") {
            var pl = opt.parallaxLevels[0] / 100, offsv = diffv * pl;
